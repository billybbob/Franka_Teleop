...
class GripperExampleController : public controller_interface::ControllerInterface {
 ...
 private:
  ...
  // Nouvelle fonction pour gérer le contrôle par trigger
  void processTriggerControl();
  // Callback pour les commandes de vélocité
  void velocityCommandCallback(const std_msgs::msg::Float32MultiArray::SharedPtr msg);
  // Callback pour les commandes de position
  void poseCommandCallback(const std_msgs::msg::Float32MultiArray::SharedPtr msg);
  ...
  // Subscription pour les commandes de vélocité
  rclcpp::Subscription<std_msgs::msg::Float32MultiArray>::SharedPtr velocity_subscription_;
  // Subscription pour les commandes de position
  ...
  // Ajouts pour le contrôle par trigger
  float trigger = 0.0;  // Variable pour stocker la valeur du trigger
  bool previous_trigger_state = false;  // État précédent pour détecter les transitions
};
