...
CallbackReturn GripperExampleController::on_configure(const rclcpp_lifecycle::State&) {
  ...
  // Create subscription to gripper pose commands
...
CallbackReturn GripperExampleController::on_activate(const rclcpp_lifecycle::State&) {
  ...
  // Initialiser l'état du trigger
  previous_trigger_state = false;
  
  // Ne plus appeler toggleGripperState() automatiquement
  // La pince sera contrôlée uniquement par le trigger
...
void GripperExampleController::poseCommandCallback(const std_msgs::msg::Float32MultiArray::SharedPtr msg) {
  // Vérifier que le tableau a au moins 8 éléments
  if (msg->data.size() >= 8) {
    trigger = msg->data[7];
    RCLCPP_DEBUG(get_node()->get_logger(), "Trigger value: %f", trigger);
    
    // Traiter le contrôle par trigger
    processTriggerControl();
    
  } else {
    RCLCPP_WARN_THROTTLE(get_node()->get_logger(), *(get_node()->get_clock()), 1000,
                         "valeur_effecteur array has only %zu elements, need at least 7 for gripper control", 
                         msg->data.size());
  }
}
...
void GripperExampleController::processTriggerControl() {
  ...
  // Détecter le changement d'état du trigger
  if (current_trigger_state != previous_trigger_state) {
    if (current_trigger_state) {
      // Trigger activé : fermer la pince
      RCLCPP_INFO(get_node()->get_logger(), "Trigger activated (%.2f > 0.05) - Closing gripper", trigger);
      graspGripper();
    } else {
      // Trigger désactivé : ouvrir la pince
      RCLCPP_INFO(get_node()->get_logger(), "Trigger deactivated (%.2f <= 0.05) - Opening gripper", trigger);
      openGripper();
    }
...
bool GripperExampleController::openGripper() {
  ...
  if (ret) {
    RCLCPP_DEBUG(get_node()->get_logger(), "Submited a Move Goal");
  } else {
...
void GripperExampleController::graspGripper() {
  ...
  if (ret) {
    RCLCPP_DEBUG(get_node()->get_logger(), "Submited a Grasp Goal");
  } else {
